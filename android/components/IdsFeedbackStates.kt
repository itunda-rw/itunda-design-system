...
@Composable
fun IdsLoadingState(label: String = "Loading", modifier: Modifier = Modifier) {
...
@Composable
fun IdsEmptyState(title: String, message: String? = null, modifier: Modifier = Modifier) {
    Column(modifier.fillMaxWidth().padding(32.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        if (message != null) Text(message, color = Ids.colors.textSecondary)
    }
}

@Composable
fun IdsErrorState(title: String, message: String? = null, retryLabel: String = "Retry", onRetry: (() -> Unit)? = null, modifier: Modifier = Modifier) {
    Column(modifier.fillMaxWidth().padding(32.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(12.dp)) {
        Text(title, color = Ids.colors.textPrimary)
        if (message != null) Text(message, color = Ids.colors.textSecondary)
        if (onRetry != null) IdsButton(text = retryLabel, onClick = onRetry, size = IdsButtonSize.Medium)
    }
}
